...
loginForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (response.ok) { // Status 200 OK
            ...
            // O backend retorna o UsuariosResponseDTO, mas o principal é a sessão.
            // Para SPA, o ideal é redirecionar para a área protegida.
            // NOTA: O recurso 'rememberMe' precisaria de lógica adicional de token (fora do escopo da sessão simples).
            window.location.href = '../pages/homeUsuario.html'; // Exemplo de página inicial
        } else if (response.status === 401) { // 401 UNAUTHORIZED (AutenticacaoException)
            // Lógica para quando o email ou senha são inválidos
